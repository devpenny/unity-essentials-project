...
public class UpdateCollectibleCount : MonoBehaviour
{
    ...
    private void UpdateCollectibleDisplay()
    {
        ...
        // Play win animation
        if (totalCollectibles == 0 && !hasWon) {
            Instantiate(winEffect, player.transform.position, player.transform.rotation);
            hasWon = true;
        }

        // Update the collectible count display
        collectibleText.text = $"Collectibles remaining: {totalCollectibles}";
        if (hasWon) collectibleText.text = "CONGRATULATIONS YOU WON!";
    }
}
